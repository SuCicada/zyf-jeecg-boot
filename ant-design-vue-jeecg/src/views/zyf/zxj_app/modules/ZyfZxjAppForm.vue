    ...
    <j-form-container :disabled="formDisabled">
      <a-form :form="form" slot="detail">
        <a-row>
          ...
          <!--          </a-col>-->
          <a-col :span="24">
            <a-form-item label="助学金" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <!--              <a-input v-decorator="['zxjId']" placeholder="请输入助学金id"  ></a-input>-->
              <a-select
                  mode="default"
                  style="width: 100%"
                  placeholder="请选择助学金"
              ...
              >
                <a-select-option v-for="(zxj,index) in selectedZxj " :key="index.toString()"
                                 :value="zxj.id">
                  {{ zxj.zxjName }}
                </a-select-option>
              </a-select>
...
export default {
  ...
  data() {
    return {
      ...
      officeCheck: 0,
      url: {
        add: "/zxj_app/zyfZxjApp/add",
        edit: "/zxj_app/zyfZxjApp/edit",
        queryById: "/zxj_app/zyfZxjApp/queryById"
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    edit(record) {
      ...
      this.$nextTick(() => {
        this.form.setFieldsValue(pick(this.model, 'studentId', 'zxjId', 'zxjName',
            'teacherCheck', 'officeCheck', 'accepted',
            'teacherCheckStr', 'officeCheckStr', 'acceptedStr'))
      })
      const getZxjList = (params) => getAction("/zxj/zyfZxj/queryall", params);
      getZxjList().then(res => {
        this.selectedZxj = res.result
      })
    ...
    popupCallback(row) {
      this.form.setFieldsValue(pick(row, 'studentId', 'zxjId', 'teacherCheck', 'officeCheck', 'accepted'))
    },
  }
}
</script>
